...
import ServicesCarousel from "../components/services/ServicePage";
import MedicalGallery from "../components/services/MedicalGallery";
import { ClipboardCheck, Package as BoxPackage, Truck as DeliveryTruck, Hospital, Globe, Phone, User } from "lucide-react";
      ...
      <div className="mt-10 flex justify-center gap-3">
        {Array.from({ length: Math.ceil(totalSlides / slidesToShow) }).map((_, i) => (
          <button
            ...
            className={`h-3 w-3 rounded-full transition-all ${i === Math.floor(index / slidesToShow) ? "bg-gradient-to-r from-emerald-400 via-teal-400 to-sky-400 scale-125" : "bg-zinc-400/40 dark:bg-zinc-600/40"}`}
          />
        ))}
      </div>
    </div>
